...
// 函数实现
void OLED_ShowStatusAndSpeeds(int st, float speed1, float speed2) // 传入两个速度
{
    char line0[20], line1[20], line2[20];
    snprintf(line0, sizeof(line0), "Status: %d", st);
    snprintf(line1, sizeof(line1), "M1: %.1f mm/s", speed1);  // 使用 speed1
    snprintf(line2, sizeof(line2), "M2: %.1f mm/s", speed2);  // 使用 speed2
